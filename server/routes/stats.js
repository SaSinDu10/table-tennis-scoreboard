...
router.get('/rankings', async (req, res) => {
    ...
    try {
        // Fetch all finished matches with all possible populated fields
        ...
        if (!finishedMatches || finishedMatches.length === 0) {
            return res.json([]); // No finished matches
        }
        ...
        for (const match of finishedMatches) {
            if (match.matchType === 'Individual') {
                ...
                // --- FIX: Compare as strings because of .lean() ---
                ...
                const winningTeamNum = match.winner; // Stored as 1 or 2
